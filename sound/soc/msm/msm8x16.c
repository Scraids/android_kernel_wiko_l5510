...
atomic_t quat_mi2s_rsc_ref;//<BEGIN><FFBAKK-105><2014.12.15>add for tfa9890-pa;yangliang
atomic_t quat_mi2s_clk_ref;//<BEGIN><FFBAKK-105><2015.02.02>add for tfa9890-pa-dsp mode;yangliang
#endif

int ext_spk_pa_gpio = -1;////LINE<><20140917><ext-amp>wangyanhui 
...
#ifdef CONFIG_PROJECT_L9010
static const char *const quatmi2s_clk_text[] = {"DISABLE", "ENABLE"};//<BEGIN><FFBAKK-105><2015.02.02>add for tfa9890-pa-dsp mode;yangliang
#endif
...
int msm8x16_quat_mi2s_clocks(bool enable)
{
	...
	int rc = 0;
	pr_err("%s: msm8x16_quat_mi2s_clocks() and enable=%d\n",__func__,enable);
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	pdata = snd_soc_card_get_drvdata(codec->card);
	pr_err("%s: enter\n", __func__);
	pr_err("%s: ucontrol->value.integer.value[0]=%ld\n",__func__, ucontrol->value.integer.value[0]);
	switch (ucontrol->value.integer.value[0]) {
	case 1:
		if (atomic_read(&quat_mi2s_clk_ref) == 0) {
             pr_err("enter>>>%s, quat_mi2s_clk_ref = %d\n", __func__,  atomic_read(&quat_mi2s_clk_ref));
			...
			pinctrl_select_state(pinctrl_info.pinctrl,pinctrl_info.cdc_lines_sus);
			pr_err("%s: sleep 100ms\n", __func__);
			msleep(100);
			pr_err("%s: sleep 100ms ends\n", __func__);
			pinctrl_select_state(pinctrl_info.pinctrl,pinctrl_info.cdc_lines_act);
		}
		ret = 0;
		break;
	case 0:
		if (atomic_read(&quat_mi2s_clk_ref) == 0) {
            pr_info("enter>>>%s, quat_mi2s_clk_ref = %d\n", __func__,  atomic_read(&quat_mi2s_clk_ref));
...
#ifdef CONFIG_PROJECT_L9010
	SOC_ENUM_SINGLE_EXT(2, quatmi2s_clk_text),//<BEGIN><FFBAKK-105><2015.02.02>add for tfa9890-pa-dsp mode;yangliang
#endif	
};
...
#ifdef CONFIG_PROJECT_L9010	
	SOC_ENUM_EXT("QUATMI2S CLK", msm_snd_enum[2],
			quatmi2s_clk_get, quatmi2s_clk_put),//<BEGIN><FFBAKK-105><2015.02.02>add for tfa9890-pa-dsp mode;yangliang
#endif
	...
	if ((!pdata->codec_type) &&
		...
		if (atomic_read(&pdata->mclk_rsc_ref) > 0) {
			...
			pr_debug("%s: decrementing mclk_res_ref %d\n",
					...
					atomic_read(&pdata->mclk_rsc_ref));
			#ifdef CONFIG_PROJECT_L9010
			if (atomic_read(&quat_mi2s_rsc_ref) > 0)//<BEGIN><FFBAKK-105><2014.12.15>add for tfa9890-pa;yangliang
...
static int conf_int_codec_mux(struct msm8916_asoc_mach_data *pdata)
{
	...
	val = ioread32(vaddr);
	#ifndef CONFIG_PROJECT_L9010
	val = val | 0x00030300;
	#else
	val = val | 0x00010002;//<BEGIN><FFBAKK-105><2014.12.15>add for tfa9890-pa;yangliang
	#endif
	iowrite32(val, vaddr);
	iounmap(vaddr);
	vaddr = ioremap(LPASS_CSR_GP_IO_MUX_MIC_CTL , 4);
...
if (atomic_inc_return(&quat_mi2s_clk_ref) == 1) {
	...
	ret = pinctrl_select_state(pinctrl_info.pinctrl,
	pinctrl_info.cdc_lines_act);
	//pinctrl_info.cdc_lines_quat_ext_act);
	if (ret < 0) {
		pr_err("%s:failed to enable codec gpios\n", __func__);
		goto err1;
	}
	} else {
		pr_err("%s: error codec type\n", __func__);
...
static void msm_quat_mi2s_snd_shutdown(struct snd_pcm_substream* substream)
{
	...
	pr_err("%s(): substream = %s  stream = %d, ext_pa = %d\n", __func__,
		 substream->name, substream->stream, pdata->ext_pa);

	if (atomic_dec_return(&quat_mi2s_clk_ref) == 0) {
	if ((!pdata->codec_type) && ((pdata->ext_pa & QUAT_MI2S_ID) == QUAT_MI2S_ID)) {
		ret = ext_mi2s_clk_ctl(substream, false);
	if (ret < 0)
		pr_err("%s:clock disable failed\n", __func__);
	if (atomic_read(&pdata->mclk_rsc_ref) > 0) {
...
//LINE<FFBAKK-1431><DATE20141231><headset button problem while playing music>zenghaihui    
       btn_low[0] = -50;
	btn_high[0] = 256; //525;   <20150120><only support one key void noise>wangyanhui 
	btn_low[1] = 526;
	btn_high[1] = 527;
	btn_low[2] = 528;
	btn_high[2] = 529;
			...
			struct msm8916_asoc_mach_data *pdata)
{
	...
	msm8x16_ext_spk_pa_init(pdev);//LINE<><20140917><ext-amp>wangyanhui
...
int get_cdc_gpio_lines(struct pinctrl *pinctrl, int ext_pa)
{
	...
	#else
	switch (ext_pa & (SEC_MI2S_ID | QUAT_MI2S_ID)) {//<BEGIN><FFBAKK-105><2014.12.15>add for tfa9890-pa;yangliang
	#endif
	case SEC_MI2S_ID:
		...
		break;
	#ifdef CONFIG_PROJECT_L9010
	case QUAT_MI2S_ID://<BEGIN><FFBAKK-105><2014.12.15>add for tfa9890-pa;yangliang
		pinctrl_info.cdc_lines_sus = pinctrl_lookup_state(pinctrl,
...
static int msm8x16_asoc_machine_probe(struct platform_device *pdev)
{
	...
	#ifdef CONFIG_PROJECT_L9010
	atomic_set(&quat_mi2s_rsc_ref, 0);//<BEGIN><FFBAKK-105><2014.12.15>add for tfa9890-pa;yangliang
...
static int msm8x16_asoc_machine_remove(struct platform_device *pdev)
{
	...
	if (gpio_is_valid(ext_spk_pa_gpio)) //LINE<><20140917><ext-amp>wangyanhui
